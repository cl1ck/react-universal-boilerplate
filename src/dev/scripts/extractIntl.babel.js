...
;(async function main () {
  ...
  try {
    ...
    await Promise.all(
      locales.map(locale =>
        loadExistingMappings(locale, localeMappings[locale], bar)
      )
    )

    bar = new ProgressBar('Extracting messages from source [:bar] (:item)', {
    ...
    await Promise.all(
      files.map(fileName =>
        extractFromFile(fileName, localeMappings, locales, bar)
      )
    )

    bar = new ProgressBar('Writing messages                [:bar] (:item)', {
      total: locales.length,
      width: 50
    })
    ...
    // write messages
    await Promise.all(
      locales.map(locale => writeMessages(locale, localeMappings[locale], bar))
    )
  } catch (e) {
    throw e
  }
